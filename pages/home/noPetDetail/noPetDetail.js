// pages/home/noPetDetail/noPetDetail.js
const request = require("../../../apis/request.js");
const config = require("../../../apis/config.js");
const dateUtiles = require("../../../utils/dateUtils.js");
const dateFormat = require("../../../utils/dateFormat.js");
Page({

  /**
   * 页面的初始数据
   */
  data: {
    banInfo: ''
  },

  /**
   * 生命周期函数--监听页面加载
   */
  onLoad: function(opt) {
    const item = JSON.parse(opt.item)
    this.setData({
      banInfo: item
    })
  },
  //查看大图
  showImageInfo(e) {
    let index = e.currentTarget.dataset.index; //图片索引
    let curUrl = this.data.banInfo.tImages[index].imageUrl; //当前点击的图片url
    let urls = this.data.banInfo.tImages.map(x => x.imageUrl) //原图列表
    wx.previewImage({ //显示原图
      urls,
      current: curUrl
    })
  },
  toBack(){
    wx.navigateBack({
      delta:1
    })
  },
  /**
   * 生命周期函数--监听页面初次渲染完成
   */
  onReady: function() {

  },

  /**
   * 生命周期函数--监听页面显示
   */
  onShow: function() {

  },

  /**
   * 生命周期函数--监听页面隐藏
   */
  onHide: function() {

  },

  /**
   * 生命周期函数--监听页面卸载
   */
  onUnload: function() {

  },

  /**
   * 页面相关事件处理函数--监听用户下拉动作
   */
  onPullDownRefresh: function() {

  },

  /**
   * 页面上拉触底事件的处理函数
   */
  onReachBottom: function() {

  },

  /**
   * 用户点击右上角分享
   */
  onShareAppMessage: function() {

  }
})